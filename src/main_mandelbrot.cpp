...
//
    ...
    context.activate();
    {
        ocl::Kernel mandelbrotGPU(mandelbrot_kernel, mandelbrot_kernel_length, "mandelbrot");
        ...
        bool printLog = false;
        mandelbrotGPU.compile(printLog);
        ...
        // результат должен оказаться в gpu_results
        {
            timer t;
            for (int i = 0; i < benchmarkingIters; ++i) {
                mandelbrotGPU.exec(gpu::WorkSize(workGroupSize, workGroupSize, width, height),
                                   results_gpu,
                                   width, height,
                                   centralX - sizeX / 2.0f, centralY - sizeY / 2.0f,
