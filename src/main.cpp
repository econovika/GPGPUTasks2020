...
int main()
{
    ...
    for (size_t platformIndex = 0; platformIndex < platformsCount; ++platformIndex) {
        ...
        report = clGetDeviceIDs(platformId, CL_DEVICE_TYPE_GPU, 1, &deviceId, nullptr);
        if (report == CL_SUCCESS)
            break;
        report = clGetDeviceIDs(platformId, CL_DEVICE_TYPE_CPU, 1, &deviceId, nullptr);
    }
    if (report != CL_SUCCESS)
    ...
    cl_mem as_buffer = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_USE_HOST_PTR, sizeof(float) * n, as.data(), &err);
    OCL_SAFE_CALL(err);
    cl_mem bs_buffer = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_USE_HOST_PTR, sizeof(float) * n, bs.data(), &err);
    OCL_SAFE_CALL(err);
    cl_mem cs_buffer = clCreateBuffer(context, CL_MEM_WRITE_ONLY, sizeof(float) * n, nullptr, &err);
    OCL_SAFE_CALL(err);
    ...
    // см. clBuildProgram
    report = clBuildProgram(program, 1, &deviceId, nullptr, nullptr, nullptr);
    ...
    // TODO 15 Скачайте результаты вычислений из видеопамяти (VRAM) в оперативную память (RAM) - из cs_gpu в cs (и рассчитайте скорость трансфера данных в гигабайтах в секунду)
    {
        timer t;
        for (unsigned int i = 0; i < 20; ++i) {
            OCL_SAFE_CALL(clEnqueueReadBuffer(queue, cs_buffer, CL_TRUE, 0, n * sizeof(float), cs.data(), 0, nullptr, nullptr));
            t.nextLap();
        }
